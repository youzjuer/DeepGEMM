#!/usr/bin/env python3
"""Same-process EP8 A/B for byte-wise versus coalesced NVFP4 SF stores."""
...
COALESCED_SF_STORE_ENV = "DG_NVFP4_MEGAMOE_COALESCED_SF_STORE"


def _set_store_variant(mode: int) -> None:
    assert mode in (0, 4)
    os.environ[benchmark.EPOCH_ENV] = "1"
    os.environ[benchmark.MODE_ENV] = "4"
    os.environ[COALESCED_SF_STORE_ENV] = "0" if mode == 0 else "1"
...
def main() -> None:
    ...
    benchmark._set_nvfp4_mode = _set_store_variant
