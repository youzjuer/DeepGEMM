#!/usr/bin/env python3
"""Same-process EP8 A/B for scalar versus vectorized NVFP4 output stores."""
...
def _set_store_path(mode: int) -> None:
    """Reuse the benchmark's Mode0/Mode4 slots for store-path A/B.

    Both slots execute Mode4. The nominal Mode0 slot selects the scalar
    store, while the nominal Mode4 slot selects the vectorized store.
    """
    assert mode in (0, 4)
    os.environ[benchmark.EPOCH_ENV] = "1"
    os.environ[benchmark.MODE_ENV] = "4"
    os.environ[STORE_ENV] = "0" if mode == 0 else "1"
...
def main() -> None:
    ...
    benchmark._set_nvfp4_mode = _set_store_path
